...
(function($, document, undefined) {
  ...
  var treeConfig = {/* properties & methods configurable*/
    ...
    selectOpen: 1, /* open the node on: 1 - click, 2 - dblclick, 3 - both 04.2017 */
...
//    nameValidate: /^[^\\\/?%*:|'\"<>]+$/, /* Windows-like test pattern */
  ...
  var txts = {
    confirmReplace: 'Remplazamos el nodo?',
    confirmDelete: 'Confirma eliminar?',
    knotExists: 'Este nodo ya existe',
    badName: "Nombre inválido. No sé puede usar \\\/?%*:'|\"<>",
    ajaxFail: 'Error!!!',
    loading: 'Cargando...'
  };
  ...
  var treeCommands = {/* core set */
    opn: {label: 'Abrir', menu: false},
    new: {label: 'Nuevo', icon: ['c_add.png', 'c_add-no.png']}, // c_new carpeta
    cmp: {label: 'Editar', icon: ['c_ren.png', 'c_ren-no.png']},
    pvw: {label: 'Vista Previa', icon: ['c_sch.png', 'c_sch-no.png']},
    // edt: {label: 'Editar', icon: ['c_ren.png', 'c_ren-no.png']},
    // sch: {label: 'Search', icon: ['c_sch.png', 'c_sch-no.png']},
    cpy: {label: 'Copiar', icon: ['c_cpy.png', 'c_cpy-no.png']},
    cut: {label: 'Cortar', icon: ['c_cut.png', 'c_cut-no.png']},
    pst: {label: 'Pegar', icon: ['c_pst.png', 'c_pst-no.png']},
    // mve: {label: 'Mover', menu: false},
    cre: {label: 'Mover', submenu: {
        'rot': {label: 'Es Raiz', icon: ['c_add.png', 'c_add-no.png']}, 
      }
    },
    ren: {label: 'Renombrar', icon: ['c_ren.png', 'c_ren-no.png']},
    del: {label: 'Borrar', icon: ['c_del.png', 'c_del-no.png']}
  };
  ...
  $.jsfiler = {
    ...
    Command: function(cmd, obj, prm) {
        ...
        if (cmd === 'sch') {
            ...
            var prm = {cmd: 'pvw', id: ids[0] };

            GetData(prm, function(response) {
                // console.log('Abriendo Preview en Ventana Nueva');
          ...
          GetData(prm, function(response) {
            ...
            var ids = $(tree.Id).jstree('get_selected');
            var prm = {cmd: 'edt', id: ids[0] };

            GetData(prm, function(response) {
                // console.log('Abriendo Editar en misma ventana');
                // console.log(response.url);
                window.location.href = response.url;
                // window.open(response.url);
            });
            ...
            var ids = $(tree.Id).jstree('get_selected');
            var prm = {cmd: 'cmp', id: ids[0] };

            GetData(prm, function(response) {
                // console.log('Abriendo Componer en Ventana Nueva');
                // console.log(response.url);
                window.location.href = response.url;
                // window.open(response.url);
            });
  ...
  var NodeInfo = function(obj, cmd) {
    ...
    // console.log($(trees[id].Id).jstree('get_type', o));
  ...
  $.fn.jsfiler = function(options) {
    ...
    var Init = function(id) {
      ...
      console.log('reb init tree rodri')
      console.log(parm)
...
//            $(e.target).jstree('set_icon', r[j], i); // replace default root icon
          ...
          // reb doble click en tree dblclick
        $(e.target).bind("dblclick.jstree", function (event) {
            // console.log('DOBLE CLIIIIIIIIIICK')
            var tree = $(event.target).jstree();
            var node = tree.get_node(event.target);
            $.jsfiler.Command('cmp', node, '');
...
//            $(tree.Id).jstree('delete_node', '#' + data.node.id);
  ...
  var CheckDnd = function(obj, parent) {
    if (parent === undefined) {
      ...
      var f = obj.length === 1; /* don't drag multiple nodes and/or root node */
    } else {
...
//        txt.push(NodeName(obj));
  ...
  var AllowCommands = function(obj) {
    ...
    for (var cmd in tree.cmdList) {
      if ((cmd === 'sch' && node.type < 2) ||
              (cmd === 'pvw' && node.type !== 0) || //REB
              (cmd === 'edt' && node.type !== 0) || //REB
              (cmd === 'cmp' && node.type !== 0) || //REB
              (cmd === 'rot' && node.type !== 0) || //REB
              (cmd === 'new' && node.type !== 3) ||
  ...
  var Response = function(arg, rsp, dat) {
    ...
    if (!rsp.status) {
      ...
      // console.log($(tree.Id).jstree(true).settings.types[1].icon)
    ...
    if (ids) {
      for (i in ids) {
        ...
        if (arg.cmd === 'cpy') {
          ...
          console.log(ids)
          window.location.href = '/admin/folder/'+ids.id+'/compose';
          // location.reload();
          return true;
        }
        if (i !== ids[i]) {
