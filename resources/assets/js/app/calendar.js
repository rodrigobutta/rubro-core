(function($) {
    ...
    moment.updateLocale('es', {
        weekdaysShort : ["Do", "Lu", "Ma", "Mie", "Ju", "Vi", "Sa"]
    });
	...
	var getMonthHtml = function(month, events) {
		...
		$('.calendar-actions', header).append('<button type="button" class="btn btn--arrow prev">Previous</button> ');
		$('.calendar-actions', header).append('<div class="month-label">' + month.format('MMMM YYYY') + '</div>');
		$('.calendar-actions', header).append('<button type="button" class="btn btn--arrow next">Next</button> ');
		...
		for(var i = 1; i < 6; i++) {
			$('.days-container', weekHeader).append('<div class="day ' + moment().locale('en').day(i).format('dddd').toLowerCase() + '">' + moment().day(i).format('dd') + '</div>');
		}
	...
	var getDateHtml = function(date, events, reservedSlots, activeMonth, currentDate) {
		...
		for(var i = 0; i < events.length; i++) {
			if(slot === reservedSlot) {
                ...
                eventContainer.append('<button class="btn__mobile open-modal"></button>');
			}
			slot++;
		}
	...
	var getEventHtml = function(event) {
		...
		var eventHtml = $('<p class="event ' + event.class + '" href="' + event.url + '" ' + (event.color != '' ? 'style="border-color: ' + event.color + ';"' : '') + '></p>');
        ...
        eventHtml.append('<span class="title">' + event.title + '</span> ' + event.desc);
		return eventHtml;
...
var setOutsideMonthWeekClass = function () {
	$('.week:not(".week-header") .days-container').each(function(index) {
		var isWeekOutsideMonth = $(this).find('.date').length === $(this).find('.date.outside-month').length;

		var $workweek = $(this).find('.date').not('.saturday').not('.sunday');
		var isWorkweekOutsideMonth = $workweek.length === $workweek.filter('.outside-month').length;

		if(isWorkweekOutsideMonth){
			$(this).addClass('week-outside-month');
		}
	});
}